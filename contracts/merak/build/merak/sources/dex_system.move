module merak::dex_system {
    ...
    use merak::events::{pool_created_event, liquidity_added_event, liquidity_removed_event};
    use merak::errors:: {
    ...
    public entry fun create_pool(schema: &mut Schema, asset1: u128, asset2: u128, ctx: &mut TxContext) {
        ...
        let pool_address = dex_functions::generate_pool_address(asset1, asset2);
        ...
        schema.pools().set(asset1, asset2, pool::new(pool_address, lp_asset_id));
        pool_created_event(sender, asset1, asset2, pool_address, lp_asset_id, lp_asset_symbol);
    }

    public entry fun add_liquidity(schema: &mut Schema, asset1: u128, asset2: u128, amount1_desired: u256, amount2_desired: u256, amount1_min: u256, amount2_min: u256, ctx: &mut TxContext) {
        let sender = ctx.sender();

        let (pool_address, lp_asset_id) = dex_functions::get_pool(schema, asset1, asset2).get();

        let reserve1 = assets_system::balance_of(schema, asset1, pool_address);
        let reserve2 = assets_system::balance_of(schema, asset2, pool_address);
        let amount1;
        let amount2;
        ...
        assets_functions::do_transfer(schema, asset1, sender, pool_address, amount1);
        assets_functions::do_transfer(schema, asset2, sender, pool_address, amount2);

        let total_supply = assets_system::supply_of(schema, lp_asset_id);
        let mut lp_token_amount;
        let min_liquidity = schema.min_liquidity()[];
        if (total_supply == 0) {
            lp_token_amount = dex_functions::calc_lp_amount_for_zero_supply(schema, amount1, amount2);
        ...
        if(schema.fee_to().contains()) {
            ...
            assets_functions::do_mint(schema, lp_asset_id, fee_to, fee);
            lp_token_amount = lp_token_amount - fee;
        };

        assets_functions::do_mint(schema, lp_asset_id, sender, lp_token_amount);
        liquidity_added_event(sender, asset1, asset2, amount1, amount2, lp_asset_id, lp_token_amount);
    }

    public entry fun remove_liquidity(schema: &mut Schema, asset1: u128, asset2: u128, lp_token_burn: u256, amount1_min_receive: u256, amount2_min_receive: u256, ctx: &mut TxContext) {
        let sender = ctx.sender();
        ...
        if(schema.fee_to().contains()) {
            ...
            assets_functions::do_transfer(schema, lp_asset_id, sender, fee_to, fee);
        ...
        assets_functions::do_burn(schema, lp_asset_id, sender, lp_token_burn);

        assets_functions::do_transfer(schema, asset1, pool_address, sender, amount1);
        assets_functions::do_transfer(schema, asset2, pool_address, sender, amount2);
    ...
    ///
    public entry fun swap_exact_tokens_for_tokens(schema: &mut Schema, path: vector<u128>, amount_in: u256, amount_out_min: u256, to: address, ctx: &mut TxContext) {
        let sender = ctx.sender();
        dex_functions::do_swap_exact_tokens_for_tokens(schema, sender, path, amount_in, amount_out_min, to);
    }
    ...
    ///
    public entry fun swap_tokens_for_exact_tokens(schema: &mut Schema, path: vector<u128>, amount_out: u256, amount_in_max: u256, to: address, ctx: &mut TxContext) {
        let sender = ctx.sender();
        dex_functions::do_swap_tokens_for_exact_tokens(schema, sender, path, amount_out, amount_in_max, to);
    }

    public fun get_amount_out(schema: &mut Schema, path: vector<u128>, amount_in: u256): u256 {
        dex_functions::validate_swap_path(schema, path);
        if(amount_in == 0) {
            return 0
        };
        let balance_path = dex_functions::balance_path_from_amount_in(schema, amount_in, path);
        let amount_out = balance_path[balance_path.length() - 1].get_balance();
        amount_out
    }

    public fun get_amount_in(schema: &mut Schema, path: vector<u128>, amount_out: u256): u256 {
        dex_functions::validate_swap_path(schema, path);
        if(amount_out == 0) {
            return 0
        };
        let balance_path = dex_functions::balance_path_from_amount_out(schema, amount_out, path);
