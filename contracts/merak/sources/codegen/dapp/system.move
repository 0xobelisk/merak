module merak::merak_dapp_system {
  ...
  use merak::merak_dapp_schema;

  use merak::merak_dapp_metadata;
  ...
  public(package) fun create(name: String, description: String, clock: &Clock, ctx: &mut TxContext): Dapp {
    let mut dapp = merak_dapp_schema::create(ctx);
    assert!(!dapp.borrow_metadata().contains(), 0);
    dapp.metadata().set(
            merak_dapp_metadata::new(
                name,
                description,
                ascii::string(b""),
                ascii::string(b""),
