module merak::merak_dex_functions {
    use std::u64;
    use std::debug::print;
    use std::ascii;
    ...
    use merak::merak_path_element;
    use merak::merak_events::{
        swap_executed_event
    };
    ...
    public(package) fun quote(amount: u256, reserve1: u256, reserve2: u256): u256 {
        merak_math_system::safe_mul_div(amount , reserve2 , reserve1 )
    }

    public(package) fun calc_lp_amount_for_zero_supply(schema: &mut Schema, amount1: u256, amount2: u256): u256 {
        let result  = merak_math_system::safe_mul_sqrt(amount1 , amount2 );
    ...
    public(package) fun validate_swap_path(schema: &mut Schema, path: vector<u256>) {
        ...
        let paths = merak_math_system::windows(&path, 2);
        paths.do!(|path| {
    ...
    /// Returns the balance of each asset in the merak_pool.
    /// The tuple result is in the order requested (not necessarily the same as merak_pool order).
    public(package) fun get_reserves(schema: &mut Schema, asset1: u256, asset2: u256): (u256, u256) {
        ...
        let balance1 = merak_assets_system::balance_of(schema, asset1, pool.get_pool_address());
        let balance2 = merak_assets_system::balance_of(schema, asset2, pool.get_pool_address());
        (balance1, balance2)
    }
    ...
    public(package) fun balance_path_from_amount_in(schema: &mut Schema, amount_in: u256, path: vector<u256>): vector<PathElement> {
        ...
        u64::range_do!(0, len, |i| {
            ...
            if (i + 1 < len) {
                ...
                balance_path.push_back(merak_path_element::new(asset1, amount_out));
    ...
    public(package) fun balance_path_from_amount_out(schema: &mut Schema, amount_out: u256, path: vector<u256>): vector<PathElement> {
        ...
        u64::range_do!(0, len, |i| {
            ...
            if (i + 1 < len) {
                let asset1 = path[i + 1];
                let (reserve_in, reserve_out) = get_reserves(schema, asset1, asset2);
                balance_path.push_back(merak_path_element::new(asset2, amount_in));
    ...
    public(package) fun credit_swap(schema: &mut Schema, amount_in: u256, path: vector<PathElement>): (u256, u256) {
        ...
        while (pos < len) {
            ...
            if(pos + 1 < len) {
                ...
                if (pos + 2 < len) {
                    ...
                    merak_assets_functions::do_transfer(schema, asset2, pool_from_address, pool_to_address, amount_out);
                } else {
                    merak_assets_functions::do_burn(schema, asset2, pool_from_address, amount_out);
        ...
        merak_assets_functions::do_mint(schema, asset1, pool_to_address, amount_in);
    ...
    public(package) fun swap(schema: &mut Schema, sender: address, path: vector<PathElement>, send_to: address) {
        ...
        merak_assets_functions::do_burn(schema, asset_in, sender, amount_in);
        let (asset_id, amount_out) = credit_swap(schema, amount_in, path);
        // Deposit the last asset to the send_to
        merak_assets_functions::do_mint(schema, asset_id, send_to, amount_out);
    }
