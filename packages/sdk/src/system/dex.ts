...
export class Dex {
  ...
  private readonly schemaModuleName = 'dex_system';
  ...
  async createPool(tx: Transaction, assetA: string, assetB: string, isRaw?: boolean) {
    const params = [
      ...
      tx.pure.address(assetA),
      tx.pure.address(assetB)
  ...
  async getAmountsOut(
    ...
    const dryResult = (await this.dubhe.query[this.schemaModuleName].get_amounts_out({
      tx,
      params
  ...
  async getAmountsIn(
    ...
    const dryResult = (await this.dubhe.query[this.schemaModuleName].get_amounts_in({
      tx,
      params
    })) as DevInspectResults;
    return this.dubhe.view(dryResult);
