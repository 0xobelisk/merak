module merak::dex_functions {
    ...
    public(package) fun do_swap_exact_tokens_for_tokens(
        ...
        let (balance_path, amount_out) = balance_path_from_amount_in(amount_in, path, dex, assets);
        print(&balance_path);
        assert!(amount_out >= amount_out_min, 0);

        swap(sender, balance_path, to, dex, assets);
        dex_swap_executed_event::emit(sender, to, amount_in, amount_out, path);
    }
    ...
    public(package) fun do_swap_tokens_for_exact_tokens(
        ...
        validate_swap_path(dex, path);
        let (balance_path, amount_in) = balance_path_from_amount_out(amount_out, path, dex, assets);
